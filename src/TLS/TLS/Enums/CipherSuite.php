...
enum CipherSuite : int{
  ...
  public function metadata(): array{
    ...
    array_shift($name); // remove TLS

    $metadata['key_exchange'] = array_shift($name);
    
    if($name[0] === 'WITH'){
      array_shift($name); // remove WITH
      $metadata['authentication'] = $metadata['key_exchange'];
    }
    else{
      $metadata['authentication'] = array_shift($name);
      array_shift($name); // remove WITH
    }

    if(in_array(end($name), ['SHA', 'SHA256', 'SHA384', 'SHA512', 'SM3', 'MD5'])){
      $metadata['hash'] = array_pop($name);
    }      
